select  mp.PersonGUID                                         as patient_patientId
     , s.code                                                 as patient_gender
     , convert(date,m.DATE_BD,103)         as patient_birthDate
     , l2.Lic                                                 as patient_generalPractitionerMoOid
...
from lbr_LaboratoryResearch lr
         ...
         join oms_mn_Journal                 mj         on mj.sourceGuid=m.uguid
         join oms_mn_Person                         mp         on mp.mn_PersonID=mj.rf_mn_PersonID
...
where lr.LaboratoryResearchID = ?
  and m.mkabid>0 and t.IDDOC in (1,2,3)